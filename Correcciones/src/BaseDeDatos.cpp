...
bool BaseDeDatos::registroValido(const Registro &r,
    ...
    // @comentario Notar que comparar t.campos() y r.campos no es O(1). 
...
bool BaseDeDatos::criterioValido(const Criterio &c,
    ...
    for (auto restriccion : c) {
        ...
        // @corregir t.campos() es un linear_set, por lo que count() es lineal. La complejidad total queda O(C * cr)
        // @comentario podrían agregar una función pública a Tabla para chequear esto en O(1)
...
BaseDeDatos::join_iterator BaseDeDatos::join(const string &tabla1, const string &tabla2, const string &campo) {
    ...
    // @corregir Están asumiendo que el índice siempre está en tabla2. Podría ser al revés.
    // @corregir Están copiando el índice, y luego guardan en el iterator referencias a esta variable local.
    Indice i = _indices[tabla2][campo];
    // @corregir BUG: se guardan más adelante una referencia a estas dos tablas que son variables locales.
    Tabla t1 = dameTabla(tabla1);
    // @corregir BUG: supongo que debe decir "tabla2" en la línea que sigue. Además, ¡la variable t2 no se usa!
    ...
    for (auto r = t1.registros_begin(); r != t1.registros_end(); ++r) {
        ...
        if (d.esNat()) {
            if (i._diccInt.count(d.valorNat())) {
                ...
                // @corregir BUG: t1 se destruye al salir de este scope
                Tabla* tRes = &t1;
